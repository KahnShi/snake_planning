...
namespace sampling_based_trajectory
{
  ...
  bool SamplingBasedTrajectory::generateTrajectory(VectorXd *sample_pos_ptr, VectorXd *sample_vel_ptr, VectorXd *sample_time_ptr, VectorXd *traj_param_ptr)
  {
    ...
    */
    ...
    for (int i = 0; i < n_samples; ++i){
      double cal = 1.0, factor = (*sample_time_ptr)[i];
    ...
    for (int i = 0; i < n_samples; ++i){
      if ((*sample_vel_ptr)[i] > -100.0 + 1.0){
        ...
        for (int j = 1; j < m_traj_order; ++j)
          A(row_id, col_id+j) = T(i, j-1) * permutation(j, 1);
      ...
      if (i >= 1 && i <= n_samples - 2){
        for (int order = 1; order < m_traj_dev_order; ++order){
          for (int j = order; j < m_traj_order; ++j){
            A(row_id, col_id+j) = T(i, j-order) * permutation(j, order);
    ...
    for (int i = 0; i < n_polynomial; ++i){
      ...
      for (int j = m_traj_dev_order; j < m_traj_order; ++j){
        for (int k = m_traj_dev_order; k < m_traj_order; ++k){
          H(id+j, id+k) = (pow((*sample_time_ptr)[i+1], j+k-2*m_traj_dev_order+1)
                           - pow((*sample_time_ptr)[i], j+k-2*m_traj_dev_order+1))
    ...
    std::cout << "\n\n PARAM SIZE: " << traj_param_ptr->rows() << "\n\n\n";
    for (int i = 0; i <m_traj_order * n_polynomial; ++i)
  ...
  Vector3d SamplingBasedTrajectory::getnOrderPointFromTrajectory(int order, double t)
  {
    ...
    for (int i = 1; i < m_n_samples; ++i){
      if (t <= (*m_sample_time_ptr)[i]){
        Vector3d pt_value;
        pt_value[0] = getnOrderPointFromTrajectory(order, t, i, m_traj_param_x_ptr);
        pt_value[1] = getnOrderPointFromTrajectory(order, t, i, m_traj_param_y_ptr);
        pt_value[2] = getnOrderPointFromTrajectory(order, t, i, m_traj_param_z_ptr);
        return pt_value;
      }
    }
  }
  ...
  {
    int id = sample_id * m_traj_dev_order;
